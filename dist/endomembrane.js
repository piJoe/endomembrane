...
var ActionAlreadyDefinedException = function ActionAlreadyDefinedException(action) {
    classCallCheck(this, ActionAlreadyDefinedException);

    this.name = 'ActionAlreadyDefinedException';
    this.message = 'An action with the name ' + action + ' is already defined.';
};

var GetterAlreadyDefinedException = function GetterAlreadyDefinedException(getter) {
    classCallCheck(this, GetterAlreadyDefinedException);

    this.name = 'GetterAlreadyDefinedException';
    this.message = 'A getter with the name ' + getter + ' is already defined.';
};

var ActionNotDefinedException = function ActionNotDefinedException(action) {
    classCallCheck(this, ActionNotDefinedException);

    this.name = 'ActionNotDefinedException';
    this.message = 'An action with the name ' + action + ' is not defined.';
};

var GetterNotDefinedException = function GetterNotDefinedException(getter) {
    classCallCheck(this, GetterNotDefinedException);

    this.name = 'GetterNotDefinedException';
    this.message = 'A getter with the name ' + getter + ' is not defined.';
};

var EndomembraneAction = function () {
    function EndomembraneAction(endomembrane, name, store, func) {
        classCallCheck(this, EndomembraneAction);
    ...
    createClass(EndomembraneAction, [{
        key: "commit",
        value: function commit() {
            ...
            if (this.store._commit(commands)) {
                //commit returns true, if store has changed
...
var EndomembraneGetter = function () {
    function EndomembraneGetter(endomembrane, name, store, func) {
        classCallCheck(this, EndomembraneGetter);
...
var Endomembrane = function () {
    /**
     * Creates a new Endomembrane instance.
     */
    function Endomembrane() {
        classCallCheck(this, Endomembrane);
    ...
    createClass(Endomembrane, [{
        key: 'registerAction',
        value: function registerAction(actionName, store, fn) {
            if (this._actions.hasOwnProperty(actionName)) throw new ActionAlreadyDefinedException(actionName);
        ...
        key: 'registerGetter',
        value: function registerGetter(getterName, store, fn) {
            if (this._getters.hasOwnProperty(getterName)) throw new GetterAlreadyDefinedException(getterName);
        ...
        key: 'call',
        value: function call(actionName) {
            var _actions$actionName;

            if (!this._actions.hasOwnProperty(actionName)) throw new ActionNotDefinedException(actionName);
            ...
            (_actions$actionName = this._actions[actionName]).call.apply(_actions$actionName, payload);
        }
        ...
        key: 'get',
        value: function get$$1(getterName) {
            var _getters$getterName;

            if (!this._getters.hasOwnProperty(getterName)) throw new GetterNotDefinedException(getterName);

            for (var _len2 = arguments.length, payload = Array(_len2 > 1 ? _len2 - 1 : 0), _key2 = 1; _key2 < _len2; _key2++) {
                payload[_key2 - 1] = arguments[_key2];
            }

            return (_getters$getterName = this._getters[getterName]).get.apply(_getters$getterName, payload);
        }
    }]);
    return Endomembrane;
}();
...
var EndomembraneStore = function () {
    function EndomembraneStore(jsObj) {
        classCallCheck(this, EndomembraneStore);

        this._store = immutable.fromJS(jsObj);
    }

    createClass(EndomembraneStore, [{
        key: '_getImmutableStore',
        value: function _getImmutableStore() {
            return this._store;
        }
    }, {
        key: '_commit',
        value: function _commit(commands) {
        ...
        key: '_executeCommand',
        value: function _executeCommand(commandArray) {
            var _store;

            var _commandArray = toArray(commandArray),
                method = _commandArray[0],
                params = _commandArray.slice(1);

            this._store = (_store = this._store)[method].apply(_store, toConsumableArray(params));
        }
    }]);
    return EndomembraneStore;
}();

var e$1 = Endomembrane;
e$1.Store = EndomembraneStore;
